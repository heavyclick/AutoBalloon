...
export function PaymentSuccessPage() {
  ...
  const handlePaymentSuccess = async () => {
    try {
      ...
      // 2. If we have session data locally, trigger download
      if (sessionData && sessionData.dimensions?.length > 0) {
  ...
  const downloadExcel = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        a.href = url;
        a.download = `${sessionData.filename || 'inspection'}_AS9102.xlsx`;
